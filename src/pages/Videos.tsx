...
import { Video as VideoIcon, Download, Share2, Trash2, X, Play } from "lucide-react";
...
export default function Videos() {
    ...
    try {
      ...
      if (response.code === 200 && response.data) {
        ...
        const videoFiles = response.data.filter(file =>
          file.type === 'file' && file.mime_type.startsWith('video/')
        );
        setFiles(videoFiles);
      } else {
        setFiles([]);
  ...
  const handleDownload = () => {
    ...
    selectedFilesList.forEach(file => {
      ...
      window.open(`/api/preview/${file.id}?token=${token}`, '_blank');
    });
  };
  ...
  const getVideoThumbnail = (file: FileItem) => {
    const token = localStorage.getItem('token');
                ...
                {files.map((file) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        className="aspect-square bg-[#fafafa] flex items-center justify-center overflow-hidden cursor-pointer relative"
                        onClick={() => setPreviewFile(file)}
                      >
                        <div className="w-full h-full bg-gradient-to-br from-[#1121d4]/20 to-[#1121d4]/5 flex items-center justify-center">
                          <VideoIcon className="w-16 h-16 text-[#1121d4]/40" />
                        </div>
                        {/* Play Button Overlay */}
                        <div className="absolute inset-0 flex items-center justify-center bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity">
                          <div className="w-16 h-16 bg-white/90 rounded-full flex items-center justify-center">
                            <Play className="w-8 h-8 text-[#1121d4] ml-1" />
                          </div>
                        </div>
                        {/* Duration Badge */}
                      ...
                      <div className="p-3">
                        ...
                        <div className="flex items-center justify-between text-xs text-[#64748b]">
                          ...
                          <span className="uppercase">{file.mime_type.split('/')[1] || 'MP4'}</span>
                        </div>
                      </div>
                    </div>
                  );
