...
export default function ChunkedUploadDialog({ open, onClose, onSuccess, parentId }: ChunkedUploadDialogProps) {
  ...
  const processFile = async (task: UploadTask, taskIndex: number) => {
    try {
      ...
      let uploadedChunks = checkResult.uploaded_chunks || [];

      updateTask(taskIndex, {
        status: 'uploading',
        totalChunks,
        uploadedChunks,
        message: `上传中 (0/${totalChunks})`,
      });
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        const progress = Math.floor((uploadedChunks.length / totalChunks) * 90); // 上传占 90%
        updateTask(taskIndex, {
          progress,
          uploadedChunks,
          message: `上传中 (${uploadedChunks.length}/${totalChunks})`,
        });
      }
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-[#e2e8f0]">
          <h2 className="text-xl font-bold text-[#0f172a]">大文件上传</h2>
          <button
            onClick={onClose}
